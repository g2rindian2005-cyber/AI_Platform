...
\q
```
> **Change `CHANGE_THIS_PASSWORD`** to something strong — you'll put the same value in `backend/.env` in Step 5.
...
| "Server error during registration" | Backend can't reach Postgres | Check `DB_*` values in `backend/.env`, confirm `sudo systemctl status postgresql` is active |
